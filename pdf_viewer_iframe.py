...
def get_server_host():
    """
    Get the appropriate host address for HTTP server.
    Returns network IP for online access, or localhost for local-only.
    """
    try:
        # Get network IP (accessible by others on same network)
        ...
        # Fallback to localhost if can't detect network IP
        return 'localhost'
...
def display_pdf_in_browser(pdf_path: str, height: int = 800):
    """
    Display PDF in iframe using Supabase Storage URL, HTTP server, or base64.
    
    Priority:
    1. If Supabase configured → Upload and use public URL
    2. Large files (>2MB) → HTTP server  
    3. Small files → base64
    ...
    # Try Supabase first (works everywhere!)
    try:
        from supabase_storage import upload_and_get_url
        
        # Upload silently
        public_url = upload_and_get_url(pdf_path)
        
        if public_url:
            # Create iframe with public URL
            iframe_html = f'''
            <iframe
                src="{public_url}"
                width="100%"
        ...
        # Supabase not configured, fall back to local methods
        pass
    ...
    if file_size_mb > MAX_FILE_SIZE_MB:
        ...
        if start_http_server():
            ...
            # Use network IP instead of localhost for online access
            server_host = get_server_host()
            pdf_url = f"http://{server_host}:{port}/{filename}"
            
            # Show URL info
            st.caption(f"🌐 Server: {server_host}:{port}")
